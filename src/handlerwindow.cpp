...
#include "nxmhandler/addbinarydialog.h"
...
HandlerWindow::HandlerWindow(QWidget* parent) : QMainWindow(parent), ui(new Ui::HandlerWindow) {
    ui->setupUi(this);
...
HandlerWindow::~HandlerWindow() { delete ui; }
...
void HandlerWindow::setHandlerStorage(HandlerStorage* storage) {
    ...
    for (auto iter = list.begin(); iter != list.end(); ++iter) {
        QTreeWidgetItem* newItem =
            new QTreeWidgetItem(QStringList() << iter->games.join(",") << QDir::toNativeSeparators(iter->executable));
...
void HandlerWindow::addBinaryDialog() {
    ...
    if (dialog.exec() == QDialog::Accepted) {
        ...
        if (!executableKnown) {
            QTreeWidgetItem* newItem =
                new QTreeWidgetItem(QStringList() << dialog.gameIDs().join(",") << dialog.executable());
            newItem->setFlags(newItem->flags() | Qt::ItemIsEditable);
...
void HandlerWindow::removeBinary() { ui->handlersWidget->takeTopLevelItem(ui->handlersWidget->currentIndex().row()); }
