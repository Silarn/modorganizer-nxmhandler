...
#include "nxmhandler/handlerwindow.h"
...
void handleLink(const QString& executable, const QString& link) {
    ::ShellExecuteW(nullptr, TEXT("open"), ToWString(executable).c_str(), ToWString(link).c_str(),
                    ToWString(QFileInfo(executable).absolutePath()).c_str(), SW_SHOWNORMAL);
}

HandlerStorage* registerExecutable(const QDir& storagePath, const QString& handlerPath) {
...
HandlerStorage* loadStorage(bool forceReg) {
    ...
    QDir globalStorage(QStandardPaths::writableLocation(QStandardPaths::DataLocation));
    ...
    QString handlerPath =
        HandlerStorage::stripCall(handlerReg.value("shell/open/command/Default", QString()).toString());
    ...
    } else if (handlerPath.endsWith("nxmhandler.exe", Qt::CaseInsensitive) && QFile::exists(handlerPath) &&
...
static void applyChromeFix() {
    ...
    if (parseError.error == QJsonParseError::NoError) {
        ...
        // toObject returns empty object if the key doesn't exist. Therefore if excluded_schemes exists,
        // protocol_handler existed as well
...
int main(int argc, char* argv[]) {
    ...
    if (args.count() > 1) {
        if ((args.at(1) == "reg") || (args.at(1) == "forcereg")) {
            if (args.count() == 4) {
                storage->registerHandler(args.at(2).split(",", QString::SkipEmptyParts),
                                         QDir::toNativeSeparators(args.at(3)), true, forceReg);
                if (forceReg) {
                    applyChromeFix();
                }
                return 0;
